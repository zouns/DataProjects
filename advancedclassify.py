...
def getlocation(address):
	...
	geolocator = GoogleV3()
	realaddress, (lat, lng) = geolocator.geocode(address)
...
def kmdistance(a1, a2):
	...
	return 1.609*((latdif**2 + lngdif**2)**.5)
...
def loadnumerical():
	...
	for row in oldrows:
		d = row.data
		data = [float(d[0]),yesno(d[1]),yesno(d[2]), float(d[5]),yesno(d[6]),yesno(d[7]), matchcount(d[3],d[8]), kmdistance(d[4],d[9]), row.match]
...
def rbf(v1, v2, gamma=20):
	...
	l = veclength(dv)
	return math.e**(-gamma*(l**2))


def nonlinearclassifier(point, rows, offset, gamma=10):
...
def SVMclf(rows, gamma=10):
	X = [row.data for row in rows]
	y = [row.match for row in rows]
